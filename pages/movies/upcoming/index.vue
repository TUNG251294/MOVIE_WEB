  ...
  <v-container>
    <div class="text-center my-4">
      <v-btn value="popular" class="mr-2 mb-2" :link=true to="/movies/popular"
        >Popular</v-btn
      >
      <v-btn value="upcoming" class="mr-2 mb-2" color='primary'
        >Upcoming</v-btn
      >
      <v-btn value="top_rated" class="mr-2 mb-2" :link=true to="/movies/top-rated"
        >Top Rated</v-btn
      >
    </div>
...
export default {
  ...
  async fetch() {
    await this.$axios
      .$get(`/movie/upcoming?page=${this.currentPage}`)
      .then((response) => {
